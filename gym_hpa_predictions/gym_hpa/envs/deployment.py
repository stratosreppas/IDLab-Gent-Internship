...
PROMETHEUS_URL = 'http://prom-kube-prometheus-stack-prometheus.kube-prometheus:9090'

# Endpoint of your Kube cluster: kube proxy enabled
HOST = "http://localhost:6443"

# TODO: Add the TOKEN from your cluster!
TOKEN = "t9h7w6.51xrefgr0vookw05"

####
def get_redis_deployment_list(k8s, min, max):
    """
    Creates the deployment values of the two redis pods, leader and follower, and stores them in a list
    """
...
def get_online_boutique_deployment_list(k8s, min, max):
    """
    Creates the deployment values of the 11 online boutique pods, and stores them in a list
    """
    deployment_list = [
        # 1
...
def get_max_traffic():
    """
    Returns the maximum Traffic value
    """
    return MAX_TRAFFIC

def convert_to_milli_cpu(value):
    """
    Converts the CPU value to milli CPU (divide by 1000000)???
    """
...
def change_usage(min, max, max_threshold):
    """
    Changes the usage value of the deployment ???
    """
...
def convert_to_mega_memory(value):
    """
    Converts the Memory value to Mega Memory (divide by 1000)???
    """
    last_two = value[-2:]
    new_value = 0
...
####
class DeploymentStatus:  # Deployment Status (Workload) - for each pod
    ...
    def __init__(self, k8s, name, namespace, container_name, container_image, max_pods, min_pods,
        ...
        #Q:what does the threshold do?
        #A: it is the threshold for the CPU and MEM usage, if the usage is above the threshold, the number of pods will increase
        #   if the usage is below the threshold, the number of pods will decrease
        #   the threshold is set to 0.75 by default, which means if the usage is above 75%, the number of pods will increase
        ...
        ### Initialize a variable that holds the predictions
        self.predictions = self.cpu_usage
    ...
    def deploy_pod_replicas(self, n, env):
        ...
        """
        env.none_counter = 0 ### restart none counter
