...
from gym_hpa_deepsets.gym_hpa.envs.deployment import get_max_cpu, get_max_mem, get_max_traffic, get_redis_deployment_list
from gym_hpa_deepsets.gym_hpa.envs.util import save_to_csv, get_cost_reward, get_latency_reward_redis, get_num_pods
...
class Redis(gym.Env):
    ...
    def __init__(self, k8s=False, goal_reward=COST, waiting_period=1):
        ...
        self.traffic = self.simulation_traffic() ### This is an object variable that contains the traffic as explained below
    ...
    def step(self, action):
        ...
        logging.info('Testing: [Step {}] | Action (Deployment): {} | Action (Move): {} | Reward: {} | Total Reward: {}'.format(
            self.current_step, DEPLOYMENTS[int(action[0])], MOVES[int(action[1])], reward, self.total_reward))
        ...
        self.info = dict(
            reward = reward, ### used for the tensorboard reward graph
        )
        ...
        if self.current_step == MAX_STEPS:
            self.none_counter = 0 ### restart the none counter
          ...
          #       'redis-leader_traffic_in': int("{}".format(obs[4])),
           #      'redis-leader_traffic_out': int("{}".format(obs[5])),
            #     'redis-leader_latency': float("{:.3f}".format(latency)),
                ...
                # 'redis-follower_desired_replicas': int("{}".format(obs[3])), ##
